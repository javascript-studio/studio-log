...
function write(ns, topic, msg, data, error) {
  ...
  (state.filters[ns] || state.transform).write(entry);
}
...
class Logger {
  ...
  filter(filter_stream) {
    state.filters[this.ns] = filter_stream;
    if (state.transform) {
      filter_stream.pipe(state.transform);
    }
  }
...
function setTransform(transform) {
  ...
  Object.keys(state.filters).forEach((ns) => {
    state.filters[ns].pipe(transform);
  });
}
...
logger.reset = function () {
  if (state.transform) {
    Object.keys(state.filters).forEach((ns) => {
      state.filters[ns].unpipe(state.transform);
    });
    state.transform.unpipe(state.stream);
